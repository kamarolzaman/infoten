...
public class TableAdapter extends ArrayAdapter <SubjectData> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        float test = Float.valueOf(String.valueOf((1.5)));
        if (position>1){
            height.getLayoutParams().height =  Math.round(v.getResources().getDimension(R.dimen.class_item_height)*1);
        }else {
            height.getLayoutParams().height =  Math.round(v.getResources().getDimension(R.dimen.class_item_height)*test);
        }
